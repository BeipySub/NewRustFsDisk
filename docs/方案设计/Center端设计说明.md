...
**规则**：`edge_code` 唯一，且是该站点的 Center 归档桶名。注册后站点名称、`edge_code` 和 Key 均不允许修改。
...
6. 任一步失败：更新任务为“导入失败”和错误信息；运输盘保持 `SEALED`，60 秒后自动重试。
...
| 页面 | 首期内容 |
|---|---|
| Edge 站点管理 | 新增、查看站点名称和编码；注册后不允许修改任何站点信息 |
...
格式化、初始化、站点管理和导入记录通过 [HTTP前后端接口协议.md](HTTP前后端接口协议.md) 执行；WebSocket 只推送硬盘、导入任务和统计变化。
